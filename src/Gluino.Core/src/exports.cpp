...
// TODO: EXPORT App* Gluino_App_Create() { return new App(); }
    ...
    EXPORT FileDialog* Gluino_FileDialog_Create(const FileDialogType type) { return new FileDialog(type); }
    EXPORT DialogResult Gluino_FileDialog_ShowDialog(FileDialog* fileDialog, const nwnd owner, FileDialogInfo* info) { return fileDialog->ShowDialog(owner, info); }

    EXPORT DialogResult Gluino_MsgBox_Show(const nwnd owner, const cstr text, const cstr title, const MessageBoxButtons buttons, const MessageBoxIcon icon) { return MsgBox::Show(owner, text, title, buttons, icon); }
}